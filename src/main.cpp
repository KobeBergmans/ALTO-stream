...
const struct option long_opt[] = {
    ...
    {"timeslice-limit",1, NULL, 'g'},
    {"nnz-threshold",  1, NULL, 'k'},
    {NULL,             0, NULL,    0}
};

const char* const short_opt = "hvi:o:b:l:z:r:m:x:d:t:s:e:f:ucpa:g:k:";
...
int main(int argc, char** argv)
{
	...
	// Options for dynamic time slicing
	IType nnz_threshold = ULLONG_MAX;
	IType timeslice_limit = 1;
	...
	while ((c = getopt_long(argc, argv, short_opt, long_opt, NULL)) != -1) {
		...
		case 'a':
			...
			break;
		case 'g':
			timeslice_limit = atoi(optarg);
			break;
		case 'k':
			nnz_threshold = atoi(optarg);
			break;
		case ':':
	...
	if ( model == CPSTREAM_ALTO || model == SPCPSTREAM_ALTO) {
		if (streaming_mode == -1) {
                  ...
                  int target_mode, int num_partitions)
{
    ...
    if (target_mode == -1) {
        for (int m = 0; m < AT->nmode; ++m) {
            printf("mode %d...", m);
            fflush(stdout);
            mttkrp_alto_par(m, factors, rank, AT, NULL, ofibs, oidx);
        }
        printf("\n");
    } else {
        mttkrp_alto_par(target_mode, factors, rank, AT, NULL, ofibs, oidx);
    }
    wtime = omp_get_wtime() - wtime_s;
...
static void Usage(char* call)
{
	...
	fprintf(stderr, "\t-u or --zero-based 	Assume input tensor is zero-based (default: one-based)\n");
